...
const CrearPaciente = ({ onClose, onSummit, isOpen }) => {
  ...
  useEffect(() => {}, [isAnyFieldEmpty, isFormDataEmpty]);
  ...
  const handleSubmit = (e) => {
    ...
    if (isAnyFieldEmpty || isFormDataEmpty || !isNumeroIdentidadValid) {
      console.error(
        "Todos los campos son obligatorios y/o el formato del número de identidad es incorrecto"
      );
    } else {
      onSummit(formData);
      onClose();
    ...
    <Modal className='add_patient' show={isOpen} onHide={onClose}>
      ...
      <Modal.Footer className='add_patient-footer'>
        <Form.Label
          ...
          style={{
            color: isAnyFieldEmpty || isFormDataEmpty ? "red" : "white",
          }}
        >
