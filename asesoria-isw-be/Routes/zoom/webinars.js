...
const { ZOOM_API_BASE_URL } = require("../../constants/index");
...
router.post("/:userId", async (req, res) => {
  ...
  try {
    const request = await axios.post(
      `${ZOOM_API_BASE_URL}/users/${userId}/webinars`,
      body,
      headerConfig
    );
    return res.json(request.data);
...
router.get("/:webinarId/registrants", async (req, res) => {
  ...
  try {
    const request = await axios.get(
      `${ZOOM_API_BASE_URL}/webinars/${webinarId}/registrants?${qs.stringify({
        status,
        next_page_token,
      })}`,
      headerConfig
...
router.put("/:webinarId/registrants/status", async (req, res) => {
  ...
  try {
    const request = await axios.put(
      `${ZOOM_API_BASE_URL}/webinars/${webinarId}/registrants/status`,
      body,
      headerConfig
    );
    return res.json(request.data);
...
router.get("/:webinarId/report/participants", async (req, res) => {
  ...
  try {
    const request = await axios.get(
      `${ZOOM_API_BASE_URL}/report/webinars/${webinarId}/participants?${qs.stringify(
        {
          next_page_token,
        }
      )}`,
    ...
    return errorHandler(
      err,
      res,
      `Error fetching webinar participants for webinar: ${webinarId}`
    );
  }
});
...
router.post("/:webinarId/registrants", async (req, res) => {
  ...
  try {
    const request = await axios.post(
      `${ZOOM_API_BASE_URL}/webinars/${webinarId}/registrants`,
      body,
      headerConfig
    );
    return res.json(request.data);
