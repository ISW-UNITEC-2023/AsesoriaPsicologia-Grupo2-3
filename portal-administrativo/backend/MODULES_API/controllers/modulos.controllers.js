...
const getModulos = async (req, res) => {
  ...
  try {
    const modulos = await modulosService.getModulos();
    res.send(modulos);
  } catch (e) {
    console.log(e);
  }
...
const createMod = async (req, res) => {
  ...
  try {
    if (typeof name == "string" && typeof description == "string") {
      const [id] = await modulosService.createMod(req.body);
      res.send({ id });
    }
  } catch (e) {
    res.status(500).send({
...
const updateModName = async (req, res) => {
  ...
  try {
    if (typeof id == "number" && typeof name == "string") {
      await modulosService.updateModName(id, name);
...
const updateModDes = async (req, res) => {
  ...
  try {
    if (typeof id == "number" && typeof description == "string") {
      await modulosService.updateModDes(id, description);
      res.send({ updatedModuloId: id });
    }
  } catch (e) {
    console.log(e);
...
const deleteMod = async (req, res) => {
  ...
  try {
    await modulosService.deleteMod(id);
